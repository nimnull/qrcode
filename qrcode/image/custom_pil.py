...
from page import get_font
from settings import INCH, RESOLUTION, SIDE, VERSIONS, TEXT_FONT, QR_FONT_SIZE
...
class PilImage(qrcode.image.base.BaseImage):
    ...
    def prepare_logo(self):
        logo = Image.open(VERSIONS[self.version]['LOGO'])
    ...
    def prepare_text_under(self, text):
        font = get_font(TEXT_FONT, QR_FONT_SIZE)
    ...
    def prepare_text_above(self):
        text = VERSIONS[self.version]['QR_HEAD_TEXT']
        font = get_font(TEXT_FONT, QR_FONT_SIZE)
        font_size = self._idr.textsize(text, font=font)
        x = self._img.size[0] / 2 - font_size[0] / 2
        y = 2
    ...
    def drawrect(self, row, col, second=False, corners=(0, 0, 0, 0)):
        if (row in [2, 3, 4] and col in [2, 3, 4]) or \
           ...
           (row in [36, 37, 38] and col in [2, 3, 4]):
            fill_color = VERSIONS[self.version]['SECOND_COLOR']
        else:
            fill_color = second and VERSIONS[self.version]['SECOND_COLOR'] \
                            or VERSIONS[self.version]['FIRST_COLOR']
